...
public class allergiespage {
	...
	private List<String> excelAllergiesIngredients;
	...
	private static final Logger logger = Logger.getLogger(allergiespage.class.getName());
	List<String>columnNamesAllergies=Collections.singletonList("Allergies (Bonus points)");
	@BeforeClass
	public void readExcel() throws Throwable {
		...
		try {
			excelAllergiesIngredients=ExcelReader.getDataFromExcel("Filter -1 Allergies - Bonus Poi",columnNamesAllergies, inputDataPath);
			logger.info("Allergies:"+excelAllergiesIngredients);
		} catch (IOException e) {
			logger.severe("Error reading Excel: " + e.getMessage());
		}
	}
	public void allergiespage(WebDriver driver, String alphabetPageTitle) throws Throwable {
		this.driver = driver;
		extractRecipes();
	}
	...
	try {
		...
		// iteration)
		DatabaseUtils.createTable("Filter1Allergies");
		...
		List<String> unmatchedLchfIngredients = getUnmatchedIngredients(excelAllergiesIngredients,
				webIngredients);
				// Sequential inserts based on conditions
		if (!unmatchedLchfIngredients.isEmpty()) {
			DatabaseUtils.insertIntoTable("Filter1Allergies", recipeID, recipeName, recipeCategory, foodCategory,
...
private List<String> extractIngredients() {
	List<WebElement> ingredientsList = driver
			.findElements(By.xpath("//div[@id='rcpinglist']//span"));
...
private void getPreparationMethod() {
	try {
		preparationMethod = driver.findElement(By.xpath("//div[@id='recipe_small_steps']/span//ol[@class='rcpprocsteps']")).getText();
